...
class OpenDartReader():
    ...
    def __init__(self, api_key):
        ...
        fn = 'opendart_mcp_corp_codes_{}.pkl'.format(datetime.today().strftime('%Y%m%d'))
        fn_cache = os.path.join(docs_cache_dir, fn)
        for fn_rm in glob.glob(os.path.join(docs_cache_dir, 'opendart_mcp_corp_codes_*')):
            if fn_rm == fn_cache:
                continue
            os.remove(fn_rm)
    ...
    def find_corp_code(self, corp):
        corp = corp.strip()
        if corp.isascii() and corp.isdigit() and len(corp) == 6:
            df = self.corp_codes[self.corp_codes['stock_code'] == corp]
        elif corp.isascii() and corp.isdigit() and len(corp) == 8:
    ...
    def finstate(self, corp, bsns_year, reprt_code='11011'):
        if ',' in corp:
            code_list = []
            for c in corp.split(','):
                code = self.find_corp_code(c.strip())
                if code is None:
                    raise ValueError(f'could not find "{c.strip()}"')
