...
USER_AGENT = 'opendart-mcp/0.2.2 Python-requests'
...
def _requests_get_cache(url, headers=None):
    ...
    fn = os.path.join(docs_cache_dir, url_hash + '.cache')
    if not os.path.isfile(fn) or os.path.getsize(fn) == 0:
        r = requests.get(url, headers=headers, timeout=30)
        tmp_fd, tmp_path = tempfile.mkstemp(dir=docs_cache_dir, suffix='.tmp')
        try:
            with os.fdopen(tmp_fd, 'wt', encoding='utf-8') as f:
                f.write(r.text)
...
def sub_docs(rcp_no, match=None):
    ...
    * match: 매칭할 문자열 (문자열을 지정하면 문서 제목과 가장 유사한 순서로 소트 합니다)
    '''
    if rcp_no.isdecimal():
        r = requests.get(f'https://dart.fss.or.kr/dsaf001/main.do?rcpNo={rcp_no}', headers={'User-Agent': USER_AGENT}, timeout=30)
    elif rcp_no.startswith('http'):
        r = requests.get(rcp_no, headers={'User-Agent': USER_AGENT}, timeout=30)
    else:
...
def attach_files(arg): # rcp_no or URL
    ...
    * rcp_no: 접수번호를 지정합니다. rcp_no 대신 첨부문서의 URL(http로 시작)을 사용할 수 도 있습니다.
    '''
    url= arg if arg.startswith('http') else f"https://dart.fss.or.kr/dsaf001/main.do?rcpNo={arg}"
    ...
    r = requests.get(download_url, headers={'User-Agent': USER_AGENT}, timeout=30)
    soup = BeautifulSoup(r.text, features="lxml")
    ...
    for tr in table.tbody.find_all('tr'):
        tds = tr.find_all('td')
        fname = tds[0].text
        flink = 'https://dart.fss.or.kr' + tds[1].a['href']
